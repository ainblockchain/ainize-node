...
| # | Fact | Where |
|---|------|-------|
| F1 | The teach trainer cannot start from a parent patch. It loads the disk table, sets `original[addr]` at first touch on the bare table, exports `before = original`. job.json has no parent field. | `qwen3.8/train/teach.py:260-267, :318, :329, :398`; `packages/node/src/teach.ts:1221-1234` |
...
| F5 | Chain order is "list order, last wins". `market.removePatch` does not re-assert other pinned patches; the watchdog re-applies per patch and can put a parent back **on top of** its child. | `packages/node/src/market.ts:693-717, :758-763, :822-855` |
...
| F7 | Parents are recorded only if the checkbox was ticked **and** the context id is LISTED at READY time; everything else is silently dropped. The dataset door hard-codes `patch_ids: []`. | `teach.ts:1621`; `packages/web/src/pages/TeachSettingsPage.tsx:99` |
...
| F9 | `royaltySplit` walks `parents[]` to depth 16 and splits the pool equally among unique ancestor **authors**; pass 2 carves seller-side contributors from a *shrinking* remainder (two 0.5 contributors get 50 %/25 %). | `packages/core/src/catalog.ts:222-292` |
...
| F13 | The dataset parser's duplicate/conflict key is the **normalised prompt**: NFC, control/bidi/zero-width stripped, whitespace collapsed, **case-sensitive**, no punctuation handling; conflict = same key, different normalised answer. `teach.py normalise()` is a *different* rule (NFKC, lower-case, all whitespace removed) used only for hit detection. | `packages/node/src/teach-dataset.ts:402-410, :559-571`; `teach.py:66-68` |
...
| F17 | The node has a `stub` teach backend (copies a fixture npz, simulated checks) and a `gradient` backend (docker exec into `flashtrain`, GPUs 4–6). | `packages/core/src/config.ts:154, :271`; `teach.ts:12, :110, :359` |
...
i18n keys are proposed names under `packages/web/src/i18n/pages/{teach,detail,chat,explore}.ts`. `{lineage}` is the network creator share in percent (from config `royaltyShare`).
...
### 5.1 `PatchAnchor` (`packages/core/src/types.ts`) — all new fields optional, ids/hashes only
...
Blob layout (new content-addressed kind, `packages/node/src/blobs.ts` accepts it beside `.npz`):
```
...
### 5.3 `teach_datasets`, `teach_jobs` (`packages/node/src/store.ts`)
...
### 5.5 Catalog lineage graph (`packages/core/src/catalog.ts`, `api.ts`)
...
|---|---|---|---|
| **L0** Prerequisites | HMAC visitor ids + `publicEvents` strip; `events` retention; `patch_signals_daily`; `royaltySplit` pass-2 fix + two-parent test + Σ-shares clamp; licence list in core; PII row status; server-side declaration + real consent; `conflicts()` skips declared base pairs; seed relabel | `packages/node/src/{market,api,store,teach,teach-dataset,seed}.ts`, `packages/core/src/{catalog,types,config}.ts`, `packages/core/test/core.test.ts`, `packages/web/src/components/chat/PublishSheet.tsx`, `docs/ux-test-scenarios.json` (AZ-254, 258, 265, 269) | [stub] |
| **L1** Anchor + dataset blob | `derivation`/`base`/`dataset.*` types, `withEmptyArrays`, 32-sample cap + `answers_hash`, `samples[].source`; dataset blob kind + manifest + snapshot at job creation + pin at publish; `/p2p/dataset`, `PeerInfo.datasets`, `mayReadDataset`, derive intents; announce validation (§12.6) | `packages/core/src/{types,ain-ledger,local-ledger,catalog}.ts`, `packages/node/src/{blobs,p2p,teach-datasets,teach,market,api,openapi}.ts` | [stub] |
| **L2** Runtime stack | `patch.py check/apply-with-journal/remove-from-journal/status`; ordered `applied`, journal dir, `applyStack`, `has_dependents`, whole-stack watchdog, chat stack builder from base stacks, `GET /api/runtime/stack`, `verifyStack` with stratified `per_source` | `qwen3.8/scripts/patch.py`, `qwen3.8/engram/live.py` (read helper only), `packages/node/src/{runtime,market,verifier,store,api}.ts`, `packages/core/src/npz.ts` (`valuesEqualCount`) | [hook] with pin/ep12/pixel fixtures (AZ-243–246, 263) |
| **L3** Trainer on-top | `teach.py` `parents`/`known_file`/`replaces`/`export`/`mask`/`probe_with_parents`, `meta` member, recipe `parents/fact_addrs/timing`, version line; node writes parents into job.json, CHECKING in deployment order + reversibility assertion, `teach_stats` populated | `qwen3.8/train/teach.py`, `packages/node/src/{teach,teach-recipe}.ts` | [gradient] on a measure-scale job (AZ-240–242); job.json shape [stub] |
| **L4** Base selection (both doors) + fork | `base_ids`/`context_ids`/`mode`, base picker sheet, basket base row, dataset-door *Start from*, inherited rows table, pre-flight `in_base`/`base_conflict`, `POST /fork`, My datasets card, publish sheet dataset section, `parent_not_listed` | `packages/web/src/components/chat/{LessonBasket,PreflightList,PublishSheet,LessonCard}.tsx`, `packages/web/src/pages/{TeachSettingsPage,TeachDatasetPage,TeachLessonPage,ChatPage}.tsx`, `packages/web/src/lib/teachStore.ts`, `packages/web/src/i18n/pages/teach.ts`, `packages/node/src/{teach,teach-datasets,api}.ts` | [stub] (AZ-234–239, 247–248, 251–255) |
| **L5** Outcomes + supersede consent | Outcome chooser, `update` with author-signed supersede, `contradict` → track creation, correction adoption, `reconcileSupersedes` limited to consented updates, schema-inheritance flag (off) | `packages/node/src/{market,teach,api}.ts`, `packages/cli/src/commands/branch.ts`, `packages/web/src/components/chat/PublishSheet.tsx`, `PatchPage.tsx` | [stub] (AZ-249–250) |
| **L6** Family tree + signals + issues | `/tree`, `/signals`, `/issues`, `/chat/feedback`, `patch_issues` + `covered_by`, Family tree tab (SVG), training-set block, doing-well strip, missing panel, chat consent, Explore shelves/sorts, operator stack + ledger meter | `packages/node/src/{api,market,store,teach}.ts`, `packages/web/src/pages/{PatchPage,ExplorePage,ChatPage,LedgerPage}.tsx`, `packages/web/src/i18n/pages/{detail,explore,chat}.ts` | [stub] (AZ-256–260) |
| **L7** Merge | `/merge/preview`, resolutions, T0 union (numpy on node), T1 `mask.only` job, T2 rebuild job, stratified verification, merge screen, `ainize patch merge` | `packages/node/src/{teach,api,market}.ts`, `packages/core/src/npz.ts`, `packages/web/src/pages/MergePage.tsx` (new), `packages/cli/src/commands/patch.ts` | T0 [hook]; T1/T2 [gradient] (AZ-261–265) |
| **L8** Buy bundle + CLI parity | 402 `requires[]`, `?bundle=1`, buy sheet, `ainize patch buy/apply/remove/stack/tree/missing/signals`, `ainize dataset get`, `teach train --on`, `teach publish` flags | `packages/node/src/{api,market}.ts`, `packages/web/src/components/BuySheet.tsx`, `packages/cli/src/{bin.ts,commands/teach.ts,commands/teach-dataset.ts,commands/patch.ts}` | [stub] (AZ-266–268) |
| **L9** Docs | D12 revision note in `teachable-dataset-design.md`, `teach-mode-design.md` §5.5/§8.3 pointers to this document, scenario rewrites listed in §14, RUN-LOCALLY template | `docs/*` , `packages/node/src/teach-recipe.ts` | — |

**Gating.** *Build on this*, the basket base row and `--on` stay hidden/disabled (feature flag `teach.lineage`) until L2 and L3 are verified end to end on GPUs 4–6 (AZ-240–246). L0, L1, L6 (read-only tree/shelves with `declared` edges) and the seed relabel can ship first. Rough sizing: L0 1 wk · L1 1.5 wk · L2 1.5 wk · L3 1 wk + GPU time · L4 2 wk · L5 1 wk · L6 1.5 wk · L7 2 wk · L8 1 wk · L9 0.5 wk ≈ 13 engineer-weeks.
