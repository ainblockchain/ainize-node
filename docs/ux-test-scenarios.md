...
- Step 4 shows the bold name + `LISTED  (yours)`, key/values `id krx-all-2761`, `price 25 AIN · per_download`, `benchmark krx-ticker-codes · 2761 queries · template/chat · collateral ≤ 0.08 nat`, `gateway http://localhost:3402/x402/patch/krx-all-2761`, `verification 2/2 passed ✓ quorum`, `body on this node yes`; an `attestations` table (VERIFIER RESULT SCORE VERIFIED ON RESTARTS STAKE AT) with two rows (node-b, node-c) `PASS  free_generation=26/26 pre_apply=1/8  vllm:Qwen3.8-Flash-Next  0  5`; `lineage` with `parents : krx-all-2761-ep12 (SUPERSEDED)` and `supersedes: krx-all-2761-ep12, krx-all-2761-ep6, pixelplus-087600`; an `address-set overlaps (A₁ ∩ A₂)` table; `branches` listing `finance/KRX-latest {"market":"KRX","version":"latest"}`
...
- Step 6 shows node-d's attestation row with `VERIFIED ON vllm:Qwen3.8-Flash-Next` (third attestation; status LISTED). If node-d had instead written `hash-only` with `benchmark: not executed …` within the first 15 minutes, the grace period is broken — report it
