...
| F12 | Blob and record gossip: `x-ngram-auth = address:ts:sig` over `purpose:ts` with 5-min skew; `/p2p/records?since=` set-reconciliation; `broadcast()` best-effort push; AIN nodes never ingest peer records (they re-read the chain every 8 s). | `p2p.ts:21-33, :70-111`; `ain-ledger.ts:204-208, :416` |
...
### 10.1 Peer protocol (extends `api.ts:851-891`; every body canonical-JSON signed, `x-ngram-auth` purpose `job:<id>:<verb>`)
...
3. `ainize status --check` on all: node-a `runtime ok · runtime_id …`; node-b/c/u `delegation: 1 executor visible`; `grep -r 8000 ~/.ngram-*/**/config.json` empty.
...
- **AZ-314** Security · Claim rate limit (10/min/address), replayed `x-ngram-auth`, expired nonce, and a `JobRequest` with a bad signature are all refused with distinct codes. [unit]
