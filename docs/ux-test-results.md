...
- **Date:** 2026-09-02 (both suites re-measured on the MERGED tree: the marketplace + guard scenarios against the demo cluster and the dataset scenarios against node-u, after `git merge` brought main and teachable-ui together at `e168c59`. This pass verified the merge rather than adding product changes.)
- **Build:** `e168c59` — the merge of main into teachable-ui (both teach doors + the runtime guard), rebuilt from source: `npm run build -w packages/core -w packages/node -w packages/cli -w packages/agent`, then `tsc --noEmit` + `vite build` for packages/web. Unit suites on the same tree: core 21/21 · node 126/126 (all but ain.test.ts) · cli 21/21 · web teacherKey 6/6 · packages/e2e `tsc -p tsconfig.check.json` clean.
- **Cluster:** live demo cluster — node-a http://localhost:3402 (web + API + seller, teach ON, publish auto, trainer backend `stub`), node-b :3403 (verifier), node-c :3404 (verifier + serving); homes ~/.ngram-cluster. **The public catalog was no longer the documented four during this pass**: a concurrent session on the same machine published `adv-unfunded` (author 0x207D8574…, author_name `adv-chain`) to the SHARED local AIN chain from a throwaway cluster (~/.ngram-pa2, :3620) and then deleted that home, so node-a now derives 5 public / 2 LISTED from the chain. See the notes below.
...
- **Specs:** packages/e2e/tests/{web-visitor,web-creator,cli-operator,agent-x402,web-crosscut}.spec.ts (the 100 scenarios; web-visitor.spec.ts also carries AZ-223/224/225/226) + {web-chat-multi,web-teach,web-teach-operator}.spec.ts (teach mode) — scenarios: docs/ux-test-scenarios.json
- **Raw results:** packages/e2e/results/merged-market.log and merged-market.json (`results/` is gitignored) — 97 passed / 15 failed / 2 flaky / 11 skipped in one 29.7-min run of every project (web, mobile, cli-api). Dataset suite: packages/e2e/results/merged-dataset.log and merged-dataset.json against node-u :3422.
- **Host:** Linux-5.15.0-130-generic-x86_64-with-glibc2.35

## Summary

**89 passed / 15 failed / 0 blocked of 104**
...
| Operator | 17 | 3 | 0 |
| Agent | 14 | 0 | 0 |
| Cross-cutting | 12 | 4 | 0 |

## All scenarios

| Id | Title | Persona | Status | Duration | Note |
|---|---|---|---|---|---|
| AZ-001 | Read the landing hero and follow the two primary calls to action | Visitor | FAIL | 4.6 s |  |
| AZ-002 | Inspect the trending card for the only verified knowledge | Visitor | FAIL | 3.6 s |  |
| AZ-003 | Browse the Explore list and read every field of a knowledge row | Visitor | FAIL | 3.3 s |  |
...
| AZ-006 | Read the Buy tab as a visitor and probe the automatic-payment address | Visitor | PASS | 1.8 s |  |
| AZ-007 | Open Live test, pick knowledge and use the sample-question chips | Visitor | FAIL | 4.2 s |  |
| AZ-008 | Run a Compare test and read the correct-answer marker and quota counter | Visitor | PASS | 12.3 s |  |
| AZ-009 | Exhaust the 20-per-hour free trial and read the quota message | Visitor | PASS | 33.3 s |  |
| AZ-010 | Audit the public record: filters, integrity card and origin → derivative map | Visitor | FAIL | 53.4 s |  |
| AZ-011 | Pick an audience card and land on the right entry point (creator card = teach the model) | Visitor | PASS | 19.7 s |  |
| AZ-012 | Copy the one-line commands and read the How-it-works / Why Ainize story | Visitor | PASS | 3.9 s |  |
| AZ-013 | Re-order Explore by each sort option | Visitor | FAIL | 42.8 s |  |
| AZ-014 | Filter Explore by model and topic and search, including the empty state | Visitor | FAIL | 2.1 s |  |
| AZ-015 | Read the Overview tab: model, verification questions, integrity and tracks | Visitor | PASS | 2.0 s |  |
| AZ-016 | Follow origins, overlap and newer-version notices in both directions (and the multi-select overlap warning in Live test) | Visitor | PASS | 2.2 s |  |
| AZ-017 | Compare all knowledge on the same subject and hit the unknown-topic 404 | Visitor | PASS | 2.9 s |  |
| AZ-018 | Use 'After only' and 'Before only' views, ask a free question and clear the conversation | Visitor | PASS | 15.0 s |  |
| AZ-019 | Stop a slow live test — and be told whether it cost a free try | Visitor | PASS | 22.7 s |  |
| AZ-020 | See who holds the shared model, and that your own test is queued behind it | Visitor | PASS | 27.4 s | turn A (compare + thinking) patched answer was not ✓ Correct — patched+thinking yields an empty answer for the trained completion prompt (model-behavior finding) |
| AZ-021 | Handle the model-server-off state on Live test and Network | Visitor | FAIL | 42.8 s |  |
| AZ-022 | Explore the Network page and try the gateway router demo | Visitor | PASS | 4.3 s |  |
| AZ-023 | Use the Docs page: copy one-liners, browse the CLI table and the API groups | Visitor | PASS | 3.1 s |  |
...
| AZ-026 | Live-test an older (superseded) version and jump to its detail page | Visitor | PASS | 6.6 s |  |
| AZ-027 | Create the operator password on first visit and land on My knowledge | Creator | PASS | 5.1 s |  |
| AZ-028 | Sign in with the operator password after being redirected from a protected page (visitors are told they do not need to) | Creator | PASS | 7.1 s |  |
| AZ-029 | Review the My knowledge table for a verified and a superseded item | Creator | PASS | 4.3 s |  |
| AZ-030 | Register a new knowledge draft from a file path on the node (signed out, /new-patch shows the two-way pre-screen first) | Creator | PASS | 8.4 s | pixelplus-test-1 was already published by an earlier run — using pixelplus-test-30 |
| AZ-031 | Publish a draft after the checklist and follow verification until the knowledge is on sale | Creator | PASS | 15.3 s | draft pixelplus-test-30 re-created with visibility:test (identical fields) before publishing · both attestations landed within 2306 ms — the intermediate Verifying state was not observable in the 5 s UI poll |
| AZ-032 | Load knowledge into the model and unload it from the manage page | Creator | PASS | 11.3 s |  |
| AZ-033 | Log out from the user menu and lose access to console pages | Creator | PASS | 3.5 s |  |
| AZ-034 | Buy verified knowledge with the node's wallet from the Buy tab and load it from Purchased knowledge | Creator | PASS | 8.8 s | buyer node-b (http://localhost:3403), knowledge pixelplus-087600 at 0.1 AIN, already purchased before: true |
| AZ-035 | Reject invalid password setup input client-side and refuse a second setup server-side | Creator | PASS | 5.8 s | no cluster node still needed setup — the client-side half runs against a private node with a real needsSetup:true (node-az035) |
| AZ-036 | Keep the sample-question editor and the benchmark JSON in sync both ways | Creator | PASS | 1.8 s |  |
| AZ-037 | Show validation errors when saving an incomplete or conflicting draft | Creator | PASS | 2.0 s |  |
| AZ-038 | Edit description, price, billing and license of a draft and save | Creator | PASS | 3.5 s |  |
| AZ-039 | Validate and save the benchmark JSON of a draft | Creator | PASS | 3.3 s |  |
| AZ-040 | Inspect the overlap check and lineage of the verified KRX knowledge (and see the same overlap warned about in Live test) | Creator | PASS | 3.5 s |  |
| AZ-041 | Run Verify now on this node and see the attestation appear | Creator | PASS | 7.2 s | the timeline renders "accuracy <free_generation>" only — the pre_apply score ("1/8" in the scenario text) is not shown |
| AZ-042 | Delete a draft with typed confirmation and see that published knowledge cannot be deleted | Creator | PASS | 5.3 s | bin.ts has no `patch forget` subcommand — the manage page advertises `ainize patch forget <id>` (docs/CLI gap) |
| AZ-043 | Filter node logs by level, expand details, load older events and read the public-record timeline | Creator | PASS | 6.8 s |  |
| AZ-044 | Save display name, payout address and notification preference on the node | Creator | PASS | 6.2 s | flaky: passed on retry (web) |
| AZ-045 | Read account identity, AIN wallet balance, sales and creator revenue share | Creator | PASS | 3.0 s |  |
| AZ-046 | Remove and re-add a connected peer node | Creator | PASS | 29.2 s |  |
| AZ-047 | Review Files & changes: pairing hint, sync, file tree and change history | Creator | PASS | 4.1 s |  |
| AZ-048 | Upload a .npz file from the browser and watch the fingerprint being computed | Creator | PASS | 3.7 s |  |
| AZ-049 | Copy the README badge snippet for the auto-pay address | Creator | PASS | 5.0 s |  |
| AZ-050 | Check the model runtime card and ask the model directly | Creator | PASS | 3.3 s |  |
...
| AZ-052 | Announce a public patch and watch node-b and node-c verify it on the real model until it is LISTED | Operator | PASS | 42.4 s | announce pre-check reported conflicts: 167 (the 4 demo bodies + the pixel copies earlier runs announced), of which 3 are this node's private drafts — a visitor is shown 164, because private drafts are redacted from public overlap answers |
| AZ-053 | Use knowledge in one line: `ainize use krx-all-2761` verifies, pays in AIN, downloads and loads it; then re-run and remove | Operator | PASS | 41.3 s |  |
| AZ-054 | Live-test knowledge from the CLI: `chat --list`, one-shot compare, `--mode`, `--thinking`, `--json`, quota footer and the interactive REPL | Operator | PASS | 46.8 s |  |
| AZ-055 | Drive the public and operator HTTP API with curl from /api/openapi.json: catalog, detail, benchmarks, info, 401 guards, login token and settings | Operator | FAIL | 521 ms |  |
...
| AZ-057 | Bring up a fourth node with `ainize init`, fund it on the local AIN chain, start it detached, peer it with the demo cluster and stop it | Operator | PASS | 13.6 s |  |
...
| AZ-059 | Add, list and remove peers on a node and watch gossip discover the other nodes | Operator | PASS | 24.8 s |  |
| AZ-060 | Inspect the catalog with `patch ls`, `patch get`, `patch records` and `patch conflicts` | Operator | FAIL | 1.4 s |  |
| AZ-061 | Register a draft with `ainize publish --no-announce`, check its visibility, reject bad inputs and delete it | Operator | PASS | 7.2 s |  |
| AZ-062 | Use a SUPERSEDED knowledge with `ainize use --no-apply` and get the newer-version note | Operator | PASS | 6.0 s |  |
| AZ-063 | Audit the shared ledger with `ledger ls`, `ledger verify`, `ledger graph` and `ledger export`, and cross-check two nodes | Operator | PASS | 5.7 s |  |
| AZ-064 | Create a branch, add knowledge, subscribe a node and route `jurisdiction=KR` to it | Operator | PASS | 14.4 s |  |
| AZ-065 | Operate the local AIN chain from the CLI: `chain status`, `chain up`, `chain fund`, `chain setup` and `wallet` | Operator | PASS | 8.5 s |  |
...
| AZ-067 | Restart the demo cluster with scripts/cluster-restart.sh and confirm data survives, peers re-gossip and the agent buyer still completes a 402 purchase | Operator | PASS | 38.0 s |  |
| AZ-068 | Publish a hidden test listing with `ainize publish --test` and confirm it stays out of public catalogs and counts | Operator | FAIL | 34.7 s |  |
...
| AZ-070 | Check the aindrive mirror: `drive status --files`, `drive sync`, `drive up` before pairing, and the changes API guard | Operator | PASS | 13.1 s |  |
| AZ-071 | Run the autonomous buyer end to end: detect the gap, pay 25 AIN via 402, verify the hash, load and restore | Agent | PASS | 12.2 s |  |
| AZ-072 | Verify the x402 402 challenge contract on the seller gateway (header, body, CORS exposure, non-seller and unknown ids) | Agent | PASS | 1.1 s |  |
| AZ-073 | Verify the settled 200 response contract and its ledger/event side effects after an ain-transfer payment | Agent | PASS | 595 ms |  |
| AZ-074 | Refuse to pay when the agent's AIN balance is below the price, then succeed after funding | Agent | PASS | 11.2 s |  |
| AZ-075 | Reject forged X-PAYMENT proofs: unknown tx hash and a real transfer that did not go to the seller | Agent | PASS | 8.9 s |  |
| AZ-076 | Reject a replayed X-PAYMENT (payment already used) and ignore stale nonces in the ain-transfer scheme | Agent | PASS | 647 ms |  |
| AZ-077 | Follow supersede marks on a keyword search, and refuse an explicitly requested superseded id | Agent | PASS | 18.5 s |  |
| AZ-078 | Skip the purchase when the model already answers correctly, and check the --max-price budget guard | Agent | PASS | 12.8 s |  |
| AZ-079 | Refuse to buy when the seller offers no payment scheme the agent is allowed to use (--pay local-credit on an AIN node) | Agent | PASS | 8.1 s |  |
| AZ-080 | Detect a tampered or corrupted patch body by sha256 before applying it to the model | Agent | PASS | 7.5 s |  |
| AZ-081 | Write and read back the on-chain access receipt after a node-side purchase (ainize use / POST buy) | Agent | PASS | 14.4 s |  |
| AZ-082 | Split the price along lineage when the source knowledge has a different author (royalty share 0.3) | Agent | PASS | 35.9 s |  |
| AZ-083 | Meter live-test hits through POST /api/chat and read them back as usage events with a per-visitor quota | Agent | PASS | 38.2 s |  |
...
| AZ-085 | Show a plain error when the node API is unreachable on every public page | Cross-cutting | PASS | 6.3 s | LedgerPage / NetworkPage have no error branch: offline they show "No records yet." and the cached node cards + a spinner, never an explicit error (matches the scenario text; flagged as a UX gap). |
| AZ-086 | Refuse to downgrade to an integrity-only attestation during the 15-minute runtime grace period | Cross-cutting | PASS | 8.4 s |  |
| AZ-087 | Switch the whole UI between English and Korean and keep the choice across reloads and pages | Cross-cutting | PASS | 15.4 s |  |
...
| AZ-090 | Reflect the model-server outage consistently on Network, Manage and My knowledge | Cross-cutting | PASS | 11.7 s |  |
| AZ-091 | Show honest loading states while a 331.7 MB knowledge is loaded, and allow cancelling | Cross-cutting | PASS | 25.6 s | AZ-091 applied time: · loaded in 2.9s · The give-up landed after the node had taken the shared lock, so the try was charged and cancel + retry dropped the quota by two — the honest half of the D3 behaviour. |
| AZ-092 | Keep every page usable at 360 px width without horizontal page scrolling, and the header free of collisions at desktop widths | Cross-cutting | PASS | 36.2 s | desktop header geometry: {"960":{"intersects":false,"headerH":81,"sameRow":true,"navNeed":715,"navWidth":738},"1024":{"intersects":false,"headerH":81,"sameRow":true,"navNeed":715,"navWidth":738},"1280":{"intersects":false,"headerH":81,"sameRow":true,"navNeed":715,"navWidth":738},"1440":{"intersects":false,"headerH":81,"sameRow":true,"navNeed":715,"navWidth":738}} · overflow per page: {"landing":{"ok":true,"scrollWidth":360,"innerWidth":360},"explore":{"ok":true,"scrollWidth":360,"innerWidth":360},"detail":{"ok":true,"scrollWidth":360,"innerWidth":360},"chat":{"ok":true,"scrollWidth":360,"innerWidth":360},"ledger":{"ok":true,"scrollWidth":360,"innerWidth":360},"docs":{"ok":true,"scrollWidth":360,"innerWidth":360}}; header items outside the 360px viewport: none · flaky: passed on retry (web) · skipped [mobile]: Pixel 5 mobile emulation scales the layout viewport away from 360 CSS px; the 360px assertions run under the web project |
| AZ-093 | Operate the Live test and sign-in entirely from the keyboard with visible focus | Cross-cutting | PASS | 12.6 s | Shift+Tab from the (now disabled) textarea landed on: button "Show 18 more" |
| AZ-094 | Expose meaningful roles and accessible names to screen readers on the core pages | Cross-cutting | FAIL | 50.2 s |  |
| AZ-095 | Format large numbers, sizes and prices consistently (270,053 entries, 331.7 MB, 25 AIN) | Cross-cutting | FAIL | 49.4 s |  |
| AZ-096 | Read the Terms page and reach the 404 pages from bad URLs | Cross-cutting | PASS | 2.6 s |  |
| AZ-097 | Show helpful empty states when a filter, search or section has nothing to display | Cross-cutting | FAIL | 43.3 s |  |
| AZ-098 | Display relative times ('5m ago') with an absolute-time tooltip that keeps ticking | Cross-cutting | PASS | 1.6 min |  |
| AZ-099 | Verify what happens to scroll position and filters on browser Back from a detail page | Cross-cutting | FAIL | 49.7 s | scrollY after Back on /ledger: 0 · Back resets the ledger to the top with "All records", Forward reopens the detail on Overview (no position/filter/tab restoration) — P2 UX finding, as described in the scenario. · scrollY after Back on /ledger: 0 · Back resets the ledger to the top with "All records", Forward reopens the detail on Overview (no position/filter/tab restoration) — P2 UX finding, as described in the scenario. |
| AZ-100 | Degrade gracefully when clipboard copy is unavailable or denied | Cross-cutting | PASS | 11.5 s | CopyButton swallows clipboard failures silently (label stays "Copy", no feedback) — P2 UX finding, as described in the scenario. |
| AZ-223 | A runaway answer is cut off with a plain explanation, not shown as an endless loop | Visitor | PASS | 42.5 s | '드': 2/6 guarded completion calls were cut as a repetition, while the chat turn was not cut at all — the chat path is the quiet one, as the scenario documents |
| AZ-224 | A sample question is sent exactly as the knowledge was trained, trailing space included | Visitor | PASS | 20.1 s |  |
| AZ-225 | A question asked while another process holds the shared model is queued, not lost, and giving up costs nothing | Visitor | PASS | 41.3 s | the queued state appeared 91 ms after the question was sent |
| AZ-226 | Ask a follow-up in Compare mode and confirm each column replays only its own earlier answers | Visitor | PASS | 19.1 s |  |
...
|---|---|---|---|---|
| AZ-103 | Teach drawer from a wrong answer → basket persists across reload | PASS | 6.4 s |  |
| AZ-104 | First train → Who gets the credit? sheet → key in localStorage → backup download → restore in a fresh browser | PASS | 339 ms | skipped [web]: no backup (earlier step skipped) |
...
| AZ-106 | Job lifecycle with backend 'stub': QUEUED → TRAINING → CHECKING → READY; events kind teach; card copy per state | SKIPPED | 4 ms | skipped [web]: no job (earlier step skipped) |
| AZ-107 | Try it now on a READY lesson: /api/chat with the draft id returns before/after | SKIPPED | 4 ms | skipped [web]: no job (earlier step skipped) |
| AZ-109 | Keep it private: token download works, sha256 matches, recipe.json and RUN-LOCALLY.md served; link expires | SKIPPED | 6 ms | skipped [web]: no job (earlier step skipped) |
| AZ-110 | Publish (review mode): PENDING_REVIEW → operator approves on the Teaching tab → ANNOUNCED → verifiers attest → LISTED; anchor carries contributors[].sig that verifies | SKIPPED | 185 ms | skipped [web]: stub trainer measured against a real serving model (teach.stubOffline is false): a copied fixture cannot teach the lesson, so it never reaches PENDING_REVIEW — the review queue runs on a node with simulated checks or a real trainer · skipped [web]: shared AIN chain — approve/announce is permanent; run against a local-ledger node for announce coverage · skipped [web]: no job (earlier step skipped) |
...
| AZ-116 | Ban by address → 403 banned; hide name → "Taught by a visitor" | SKIPPED | 87 ms | skipped [web]: no announced lesson (earlier step skipped) |
...
| TM-090 | Load up to three knowledges together in one live test, see the overlap warning, and meter one usage event per knowledge | PASS | 11.4 s |  |
| TM-091 | Show the contamination banner when the operator keeps knowledge loaded for everyone | PASS | 5.5 s |  |
...
- **All 15 failures have one cause outside the code, and it is not the merge.** A concurrent session published `adv-unfunded` to the shared local AIN dev chain (`/apps/knowledge/market/patches/adv-unfunded`) and then deleted the node home holding its key. node-a derives its catalog live from that chain, so the demo fixture went from the documented 4 public / 1 LISTED to 5 public / 2 LISTED and the anchor sorts to the top of Explore. Every one of the 15 reads back as that single fact — `Expected 4, Received 5` (AZ-094, AZ-013, AZ-021, AZ-010, AZ-055, AZ-060), `Expected "krx-all-2761", Received "adv-unfunded"` (AZ-001, AZ-007), `Expected 1, Received 2` LISTED (AZ-002, AZ-003), and the same extra row in AZ-014, AZ-068, AZ-095, AZ-097, AZ-099. No assertion was weakened and no scenario was rewritten to accommodate it: the anchor cannot be removed or re-tagged from here — the chain rule on that path is author-only (a write as node-a, which IS the /apps/knowledge app admin, is rejected with code 12103), and the author key is gone with the deleted home. Clearing it needs either that key or a reseeded dev chain (`--fresh`), which this task is not allowed to do.
- **Four expectations the merge genuinely changed, fixed in the specs and in `docs/ux-test-scenarios.json` (re-rendered), never by weakening an assertion.** (1) AZ-023: the CLI docs group is now "Teach mode (turn your own questions and answers into knowledge)" — the dataset door renamed it; the scenario text, which still predated teach mode, also gained the group and the `Teach` API tag it had never listed. (2) AZ-055: the dataset door documents 11 more paths, so `/api/openapi.json` is 87 = 53 marketplace + 34 teach (was 76 = 53 + 23). (3) AZ-009 / AZ-066 / AZ-083: the 429 body carries the machine-readable `quota_chat:` prefix that the teach UI switches on (`mapTeachError`, spec §5.14 "code = message prefix"); the chat UI keys off status 429 so no visitor ever sees the code. (4) AZ-007 / AZ-026: the chat page now also renders the dataset basket, so the `main h2` locator that already excluded "Knowledge to load" and "Your lesson" excludes "Your dataset" too. All four are green in this run (AZ-023, AZ-009, AZ-026, AZ-066, AZ-083 pass outright; AZ-007 and AZ-055 get past the changed assertions and fail only on the catalog above).
...
- **The three defects the owner reported are gone, proved on the live cluster.** (a) `드` no longer runs away and is no longer auto-scored — it comes back as an ordinary answer marked *Free question — not auto-scored*, and where the model does loop the reply is cut with one plain sentence and a *Show the raw answer* button that reveals the full text unchanged (AZ-223). (b) The sample chip `종목코드 픽셀플러스 ` puts the trained prompt in the box with its trailing space, sends it verbatim and scores `087600` as ✓ Correct (AZ-224). (c) The same question typed by hand, without the trailing space, still answers correctly. (d) A question asked while another process holds the shared model says so within ~1 s, names the holder and ticks, and *Stop waiting* returns HTTP 499 without charging a free try (AZ-225).
...
**The merge of main and teachable-ui, verified by this pass**

- `e168c59` merge main into teachable-ui: one codebase with both doors and the runtime guard
...
- `d345e55` docs: second UX critique — CLI user, node operator, knowledge publisher
- `3a4379a` docs: UX scenario results re-measured on an unchanged build — 104/104
- `d503f4f` e2e: the probes that re-measured the 17 fixed findings on the running product
- `d5bed50` docs: mark the 17 re-verified findings fixed, each with what shipped
- `fe0dba7` docs: UX scenario results re-measured on the wave-1 build — 104/104 in one run
- `3f93b0c` e2e: the two live-model scenarios that still expected the old lesson headline (finding 6)
...
- `6344b9d` teach: a simulated check says it is simulated, and a demo run stops calling itself a lesson (findings 7, 6)
...
- `e42148f` teachable: the dataset e2e suite goes green (AZ-123…AZ-222)
- `40c5820` teachable: fix what the dataset scenarios found (AZ-125..AZ-220)
- `3b20ad7` e2e: the file door, AZ-123..AZ-142 (dataset upload and preview)
- `558b4fc` docs: 100 UX scenarios for dataset-first teach mode (AZ-123..AZ-222)
- `774f2ed` teachable PR-D4: smoke both doors and fix what the walk broke open
- `14347ef` teachable PR-D3: the file door from the terminal (CLI + docs)
- `cc12d24` teachable PR-D2: the Teachable-NLP-style teach UI (upload → preview → train → result)
- `5711198` teachable PR-D1: the dataset pipeline (core + node)
- `df533d3` docs: dataset-first teach mode (Teachable-NLP-style) design
