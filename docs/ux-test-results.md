...
- **Specs:** packages/e2e/tests/{web-visitor,web-creator,cli-operator,agent-x402,web-crosscut}.spec.ts (the 100 scenarios; web-visitor.spec.ts also carries AZ-223/132/133/134) + {web-chat-multi,web-teach,web-teach-operator}.spec.ts (teach mode) — scenarios: docs/ux-test-scenarios.json
...
| AZ-223 | A runaway answer is cut off with a plain explanation, not shown as an endless loop | Visitor | PASS | 38.4 s | '드': 2/6 guarded completion calls were cut as a repetition, while the chat turn was not cut at all — the chat path is the quiet one, as the scenario documents |
| AZ-224 | A sample question is sent exactly as the knowledge was trained, trailing space included | Visitor | PASS | 19.8 s |  |
| AZ-225 | A question asked while another process holds the shared model is queued, not lost, and giving up costs nothing | Visitor | PASS | 41.2 s | the queued state appeared 91 ms after the question was sent |
| AZ-226 | Ask a follow-up in Compare mode and confirm each column replays only its own earlier answers | Visitor | PASS | 19.5 s |  |
