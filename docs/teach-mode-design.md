...
- Making the visitor the **on-chain author**. The AIN write rule `auth.addr === newData.author` on `/apps/knowledge/market/patches/$id` (*verified* `ainize-core/src/ain-ledger.ts`) forces `author = node`. The visitor is a signed **contributor**.
...
All strings live in a new `ainize-web/src/i18n/pages/teach.ts` (en + ko) plus edits to `public.ts` (landing) and `operator.ts` (sign-in notice, dashboard tab). Placeholders in `{}`.
...
> loaded is labelled *comparison only*. Keys: `teach.base.*` in `ainize-web/src/i18n/pages/teach.ts`.
...
- **Visitor auth:** header `x-ainize-auth: <address>:<ts>:<sig>` where `sig = signMessage("teach:<ts>")` with the browser key; verified by the existing `verifyAuthHeader(header, 'teach')` (*verified* `ainize-node/src/p2p.ts:27`, 5-min skew). Browser signing uses `@noble/secp256k1` + keccak with the same personal-message format `ainize-core/src/identity.ts` verifies through ain-util (`packages/web` has no ain-util dependency).
...
→ 200 { job: TeachJob }   full body only when x-ainize-auth = job.contributor.address or operator;
...
### 7.2 `ainize-core/src/ain-ledger.ts` — required correction
...
### 7.3 `ainize-core/src/catalog.ts` — `royaltySplit` two-pass
...
### 8.3 Worker state machine (`ainize-node/src/teach.ts`, modelled on `Verifier`)
...
- Browser generates a secp256k1 key pair on first **Train this lesson**; address derived like `ainize-agent/src/identity.ts` / `ainize-core/src/identity.ts` so it is a valid AIN address and the node verifies with existing code. Stored only in `localStorage`; exportable/importable as JSON. No password, no e-mail, no operator session.
- Every visitor teach request is signed (`x-ainize-auth`, 5-min replay window). A stolen lesson URL only reveals status.
- At publish the browser additionally signs `hashCanonical({patch_sha256, benchmark_hash, address, share})` (*verified* `ainize-core/src/canonical.ts:66`) and the node writes it into `contributors[].sig` with `proof: 'signed'`. A typed payout wallet that is not the signing key is recorded with `signer = key address`, `address = wallet`, `proof: 'declared'` — it can only receive money, never claim someone else's lesson.
...
`RUN-LOCALLY.md` is rendered server-side by `ainize-node/src/teach-recipe.ts` with `{model_id, sha256, filename, download_url, first_prompt, parents}` filled in:
...
| identity | signed `x-ainize-auth` on every write; signed claim in the anchor; owner-only reads of full job bodies |
...
- **AZ-011** landing cards: creator card title/steps/CTA now §5.1; CTA lands on `/chat?teach=1`, not `/signing`. Update `ainize-node/e2e/tests/web-visitor.spec.ts` (landing block around L525).
...
`ainize-core/src/types.ts` (Contributor, anchor/recipe fields) · `ainize-core/src/catalog.ts` (two-pass royaltySplit) · `ainize-core/src/ain-ledger.ts` (explicit `contributors ??= []`, cap 4) · `ainize-core/src/config.ts` + `types.ts` NodeConfig (`teach`, `market.contributorShare`) · `ainize-core/test/core.test.ts` · `ainize-node/src/market.ts` (createDraft/updateDraft accept `contributors`, `origin`, `visibility`) · `ainize-node/src/api.ts` (`GET /api/info` fields, `GET /api/catalog?contributor=`) · `ainize-node/src/openapi.ts`.

**PR-2 — node: payouts table + earnings**
`ainize-node/src/store.ts` (`payouts`, `contributors`) · `ainize-node/src/market.ts` (settlePayment writes payouts before transfer, retry timer) · `ainize-node/src/server.ts` (60 s retry interval) · `ainize-node/src/api.ts` (`GET /api/teacher/:address`, `GET /api/me/payouts`, retry) · `ainize-cli/src/commands/branch.ts` (wallet shows pending) · tests.

**PR-3 — ChatMode multi-knowledge + contamination banner**
`ainize-node/src/market.ts` (chat `patchIds[]`) · `ainize-node/src/api.ts` (`/api/chat` schema, `/api/chat/patches.applied`) · `ainize-web/src/components/chat/KnowledgePicker.tsx` (checkboxes, overlap, banner) · `ChatPage.tsx`, `api/api.ts`, `api/types.ts`, `i18n/pages/chat.ts` · `ainize-cli/src/commands/chat.ts` (`--patch a,b`) · e2e AZ-090/091 · `docs/ux-test-scenarios.md`.
...
`ainize-node/src/teach.ts` (worker, slot lease, docker exec bridge, stdout parser, CHECKING with locality/parent gates, stub backend) · `ainize-node/src/runtime.ts` (`exclusiveTry`) · `ainize-node/src/store.ts` (`teach_jobs`, `teach_quota`, `teach_stats`, `bans`) · `ainize-node/src/api.ts` (all `/api/teach/*` visitor routes, `/api/me/teach/*`) · `ainize-node/src/teach-recipe.ts` (recipe.json, RUN-LOCALLY.md) · `ainize-node/src/server.ts` (start/stop worker, expiry sweep) · `ainize-node/src/p2p.ts` (no change; reuse `verifyAuthHeader`) · unit tests with fake spawn · e2e AZ-094–099, 104–109 with `backend: 'stub'`.

**PR-6 — web: teach flow**
`ainize-web/src/lib/teacherKey.ts` (`@noble/secp256k1` + keccak, sign/verify compat test) · `components/chat/TeachDrawer.tsx`, `LessonBasket.tsx`, `CreditSheet.tsx`, `PreflightList.tsx`, `LessonCard.tsx`, `PublishSheet.tsx`, `KeepPrivateSheet.tsx`, `MyKnowledgePanel.tsx` · `pages/TeacherPage.tsx` (`/teacher/:address`) · `pages/ChatPage.tsx` (query params, sticky card) · `pages/PatchPage.tsx` + `PatchListItem` ("Taught by", "Use it yourself") · `api/api.ts` (RTK endpoints, 5 s polling) · `i18n/pages/teach.ts` (en+ko) · `App.tsx` routes.

**PR-7 — landing, sign-in, operator tab**
`pages/LandingPage.tsx` · `i18n/pages/public.ts` · `pages/SigningPage.tsx` + `components/base/Layout.tsx` (`/new-patch` pre-screen) · `i18n/pages/operator.ts` · `pages/DashboardPage.tsx` (Teaching tab) · `pages/AccountPage` settings · `ainize-node/e2e/tests/web-visitor.spec.ts` (AZ-011/028/030) · `docs/ux-test-scenarios.md` (+ AZ-090…111) · `README.md` (visitor teach section, local-run link).

**PR-8 — CLI parity + docs**
`packages/cli` (`ainize teach status`, `ainize patch import`, `publish --contributor addr:name:share`) · `ainize-node/src/openapi.ts` CLI_REFERENCE · deploy/README (docker group requirement, GPU allocation note) · demo cluster config (`teach.enabled: true`, `publish: 'auto'`).
...
- Containers: `ainize-ain` up 2 h, `flashtrain` up 5 h (running `python3 /work/train/train_rev.py --init-patch /work/results/train-all/rows-pin.npz --epochs 6`), `flashnext` **up 20 seconds** (just restarted; GPUs 0–3 momentarily at 0 MiB).
...
- `Store.putToken`/`checkToken` and `/p2p/blob/:sha?token=` exist; `runtime.verify(npz, bench, {restore?, maxSamples?})`; `hashCanonical` in `ainize-core/src/canonical.ts`; `verifyAuthHeader(header, purpose, maxSkewMs)` in `ainize-node/src/p2p.ts`.
...
- **`NodeConfig.teach` is optional in the type; `teachConfig(cfg)` returns the effective config.** Configs written before teach mode have no `teach` block; `loadConfig()` fills it (`DEFAULT_TEACH_CONFIG`, deep-merged for `trainer` / `locality`) so `config.json` on disk shows the defaults after the next save, and in-memory literals (tests, `scripts/cluster.mjs`) keep compiling. Env overrides `AINIZE_TEACH_BACKEND=stub|gradient` and `AINIZE_TEACH_ENABLED=0|1` were added to `applyEnv` for CI/e2e.
...
- **`GET /api/chat/patches`**: `applied: string[]` (operator-pinned ids from the `applied` table, `Market.pinnedPatchIds()`), **`overlaps: {a,b,rows}[]`** (pairwise `intersectionCount` of the testable items' addrsets — added so the picker and the CLI need no extra round-trips; §6.3 only said "from the existing addrsets/conflicts() logic"), and with a verified `x-ainize-auth` of purpose **`teach`**: `lessons: []` (shape only, PR-5 fills it) plus `teacher: <address>`.
...
- **UX scenarios**: §13.2 numbers the new scenarios AZ-090+, but `docs/ux-test-scenarios.md` already has AZ-090…AZ-100; the two PR-3 scenarios are filed as **TM-090 / TM-091** (spec number kept, `TM` = teach mode) in a new section, `docs/ux-test-scenarios.json` appended; `ainize-node/e2e/tests/web-chat-multi.spec.ts` covers both and skips on a pre-teach node (no `applied[]`). Later PRs should continue with TM-092….
- **Tests**: `ainize-node/test/chat.test.ts` (fake runtime: order, reverse restore, pinned re-assert, single lock label, one usage event per patch, HTTP validation, `applied[]` / `overlaps[]` / `lessons[]`); `ainize-cli/test/cli.test.ts` (parsePatchIds, multi renderChat, `patch_ids` request path).

### PR-5 — node: TeachWorker + visitor teach API (2026-08-31)

Files: `ainize-node/src/teach.ts` (worker), `teach-recipe.ts` (recipe.json + RUN-LOCALLY.md), `runtime.ts` (`exclusiveTry`), `store.ts` (`teach_jobs`, `teach_quota`, `teach_stats`, `contributors`, `bans`, `payouts`), `market.ts` (`teach()` = config ← kv `settings.teach`, `teachSettings()`, `updateTeachPolicy()`), `api.ts` (all `/api/teach/*`, `/api/teacher/:address`, `/api/me/teach/*`), `server.ts` (worker start/stop, `teachHooks` for tests), `openapi.ts`, `ainize-core/src/npz.ts` (`readNpzMember`, `writeNpz` for the stub), `ainize-node/test/teach.test.ts` (fake spawn replaying the §8.2 protocol incl. `error` and a hang → timeout, fake docker/nvidia-smi, fake serving runtime).
...
### PR-7 — landing, sign-in, `/new-patch` pre-screen, operator Teaching tab, UX scenarios (2026-08-31)

Files: `ainize-web/src/pages/LandingPage.tsx` (creator card = §5.1 copy, CTA → `/chat?teach=1`, secondary operator link → `/signing?next=%2Fnew-patch`, landing nav **Teach** next to Live test when `/api/info.accepts_contributions`) · `i18n/glossary.ts` (`AUDIENCE.creator` title/help now the teach wording) · `i18n/pages/public.ts` (`landing.audience.creator.title/s1/s2/s3/cta/operator_link/off`, `landing.nav.teach`) · `pages/SigningPage.tsx` (`op.sign.login.subtitle`, visitor notice + "Go to Live test →") · `components/base/Layout.tsx` `NewPatchGate` + `components/operator/NewPatchPreScreen.tsx` (§5.2 two-way pre-screen) · `App.tsx` (`/new-patch` uses the gate) · `components/ui/Header.tsx` (**Register a knowledge file** under the operator menu, `nav.register`) · `pages/DashboardPage.tsx` (tabs **My knowledge** / **Teaching**, `?tab=teaching`) · `components/operator/TeachingTab.tsx` (§5.13: settings → `PATCH /api/me/teach/policy`; queue with Approve and announce / Decline + reason / Cancel; contributors with Hide name / Block key / Block IP / Unblock + the bans table; payouts Owed / Paid / Failed + Retry → `POST /api/me/payouts/:id/retry`) · `pages/AccountPage.tsx` (a "Teaching" section that points at the tab) · `i18n/pages/operator.ts` (`op.sign.*`, `op.new.prescreen.*`, `op.account.teach.*`, `op.dash.tab.*`, `op.teach.*`, en + ko) · `i18n/pages/common.ts` (`nav.register`) · `api/api.ts` + `api/types.ts` (operator endpoints `teachAdminPolicy`, `updateTeachAdminPolicy`, `teachAdminJobs`, `approveTeachJob`, `rejectTeachJob`, `cancelTeachJobAdmin`, `teachContributors`, `setContributorHidden`, `teachBans`, `addTeachBan`, `deleteTeachBan`, `payouts`, `retryPayout`; tags `TeachAdmin`, `Payouts`) · `ainize-node/src/market.ts` (`updateTeachPolicy` fix, see below) · `ainize-node/test/teach.test.ts` (+ assertions) · `ainize-node/e2e/tests/web-teach-operator.spec.ts` (new: AZ-011 / AZ-028 / AZ-030 / settings / AZ-110 / AZ-116 / AZ-113) · `ainize-node/e2e/tests/web-teach.spec.ts` (titles renumbered to the filed ids) · `docs/ux-test-scenarios.json` (+ `scripts/render-ux-scenarios.py` now renders the `.md` as well as the `.html`) · `README.md`.

Deviations / clarifications:

- **Scenario ids.** §13.2 numbers the new scenarios AZ-090…AZ-111, but `docs/ux-test-scenarios.json` already had AZ-090…AZ-100 (and PR-3's TM-090/TM-091). They are filed **in spec order as AZ-101…AZ-122** (AZ-101 = spec AZ-090 … AZ-122 = spec AZ-111); TM-090/TM-091 stay as the detailed two-knowledge walkthroughs and AZ-101/AZ-102 reference them. PR-6's Playwright titles were renumbered to match (AZ-101→AZ-103, 102→104, 103→105, 104→106, 105→107, 106→109, 107→"AZ-110/111", 108→AZ-120, 109→"AZ-104 (restore)"); screenshot file names were left alone. §13.1: AZ-011 / AZ-028 / AZ-030 / AZ-016 / AZ-040 updated as written; AZ-007 / AZ-066 / AZ-083 got the one-line "picker is checkboxes / `patch_id` still valid, `patch_ids` for several" wording. Personas "Teach mode (visitor)" and "Teach mode (operator)" were added rather than spreading the scenarios over the old personas.
- **`docs/ux-test-scenarios.md` is now generated** from the JSON by `scripts/render-ux-scenarios.py` (the task brief assumed the script already did this; it only rendered the HTML). The generator reproduces the previous hand-written body byte for byte for AZ-001…AZ-100; PR-3's richer TM-090/TM-091 text was copied into the JSON so nothing was lost. The "How to use" intro lives in the script (`MD_INTRO`) and gained a teach-mode bullet (dev node, stub backend, spec commands).
- **`updateTeachPolicy` bug fixed (node).** The PR-5 implementation spread the whole zod-parsed body over the stored overrides, so every `PATCH /api/me/teach/policy` deleted the overrides it did not mention (the dev node's 50/500 quotas vanished on the first save from the Teaching tab). Now only keys that are present change; `null` clears one override (back to config.json), `undefined` leaves it alone. Unit test extended (`quotas and bans`).
- **Where the settings live.** §14 lists "`pages/AccountPage` settings"; §5.13 puts them on the Teaching tab. They are on the Teaching tab (one place, next to the queue they govern); Account settings only got a short "Teaching" section that links to `/dashboard?tab=teaching`. The tab also exposes **Days an unsaved lesson is kept** (`draft_ttl_days`) and **Blocked topics** (regex), both already accepted by the API. The share slider is 0–90 % in 5 % steps (API max 0.9). The form shows the *effective* values (config.json ← kv) and sends only the fields that changed.
- **Queue actions.** Cancel is offered for QUEUED/PREFLIGHT/LOADING/TRAINING/EXPORTED/CHECKING; Decline asks for the reason inline (required, ≤ 500 chars) and the contributor sees it as `reject_reason`; ANNOUNCED rows link to the knowledge page. The queue polls every 5 s while a job is active, otherwise every 30 s. Operators see the contributor's key and last IP; **Block IP** uses the last IP seen on that key's jobs and is disabled (with a hint) when none was recorded.
- **Payouts tab.** Tiles show the counts from `GET /api/me/payouts.summary` (pending = "Owed"); a local-record node shows the no-chain-wallet hint. Retry is only exercised with a mocked failed row in the e2e (the dev node runs a local ledger, so no real payout can fail there — see AZ-113 for the AIN path, covered by `ainize-node/test/payouts.test.ts`).
- **Register moved under the operator menu** as "Register a knowledge file" (§11 "Register moves under the operator menu"); the "Register knowledge" button on My knowledge stays (it is an operator page already). The landing's creator card shows "This node is not accepting lessons right now. Live test still works." when `accepts_contributions` is false, and the pre-screen's card A says the same — the CTA still opens Live test.
- **Sign-in page notice** is rendered for both the setup and the login form (a visitor who lands on a brand-new node should not create an operator password by mistake).
- **e2e.** `web-teach-operator.spec.ts` logs in through the real `/signing` page with `AINIZE_PASS` (default `teach-pass`), trains its lessons through the API with a fresh key (the browser flow is `web-teach.spec.ts`), switches the node to `publish: review` for the review test and restores the previous publish mode and quotas in `afterAll`. It needs no model server. Both teach specs: `AINIZE_URL=http://localhost:3412 AINIZE_PASS=teach-pass npx playwright test tests/web-teach*.spec.ts --project=web`.
- Not in this PR: CLI parity (`ainize teach status`, `ainize patch import`, `publish --contributor`) and the deploy/README docker-group + GPU notes (PR-8); the demo cluster config is untouched.


### PR-8 — CLI parity, docs, demo config (2026-08-31)

Files: `ainize-cli/src/commands/teach.ts` (new: `ainize teach status`), `ainize-cli/src/commands/patch.ts` (`parseContributors`, `draftFromRecipe`, `patchImport`, `PublishArgs.contributor`), `ainize-cli/src/bin.ts` (`teach status [target] [--key --key-file]`, `patch import <file> --recipe`, `--contributor` on `publish` and `patch publish`), `ainize-cli/src/index.ts`, `ainize-cli/test/cli.test.ts` (+3 tests, node started with `teach.enabled` + stub + `stubOffline`), `ainize-node/src/openapi.ts` (`CLI_REFERENCE`: teach one-liner, "Teach mode" group, `--contributor` line), `ainize-node/src/api.ts` (`PATCH /api/patches/:id` fix, below), `ainize-web/src/pages/DocsPage.tsx` + `i18n/pages/docs.ts` (fourth one-liner card "Teaching the model", en+ko), `deploy/README.md` (docker group + GPU allocation), `scripts/cluster.mjs` (teach demo config + `TEACH_BACKEND` switch), `README.md` (terminal lines), `docs/ux-test-scenarios.json` (AZ-121 rewritten with the real commands; md/html re-rendered).

Deviations / clarifications:

- **`ainize teach status <target>` accepts more than a node URL.** §6.4 says `ainize teach status <url>`; the CLI reads whatever the visitor has in hand: a node URL → `GET /api/info` + `GET /api/teach/policy` (accepting lessons?, publish mode, trainer state + reason, backend, queue depth/ETA, p50/p90, quotas, data-provider share, model, pinned ids, draft TTL); a lesson URL (`…/chat?lesson=<id>`, `…/api/teach/jobs/<id>`) or a bare job id → `GET /api/teach/jobs/:id`; a teacher page (`…/teacher/<address>`) or a bare address → `GET /api/teacher/:address`. Without a target it uses the context node.
- **Teaching key in the CLI.** `--key-file <ainize-teaching-key-….json>` (the browser's backup file, same shape as `teacherKeyBackup`), `--key <64-hex>` or `AINIZE_TEACH_KEY` (hex or a path). The CLI signs `x-ainize-auth` exactly like the web app (`<address>:<ts>:<sig over "teach:<ts>">`, core `signMessage`) — with it the lesson view is the owner's full body (facts, before/after, checks, draft/published id) and the node view lists "your lessons on this node"; without it the node returns status only (§6.1) and the CLI says so. The CLI never creates a key: teaching stays a browser flow (`/chat?teach=1`), as in §4.
- **`ainize patch import` mapping.** `createDraft` via `POST /api/patches` with `path` (the node registers the file where it is — `keepInPlace`), then `PATCH {origin:'teach'}`. From `recipe.json` (the served shape = trainer recipe + `lesson` block + `benchmark`): id = `lesson.draft_id` (`taught-<slug>`) or `taught-<file slug>` (`--id` overrides), name = `lesson.name` or `Lesson: <first prompt>`, model = `model_id` / `lesson.model_id` / `model.id_M` (`--model` when the recipe names none), benchmark = `recipe.benchmark` or, for a trainer-only recipe, built from `benchmark_samples` (schema `taught/<slug>`, format `[template, chat]`, collateral 0.08), parents = `lesson.context_patch_ids` only when `builds_on_context` and the parent exists on this node (a warning otherwise), description = the corrections + source node/job. The data provider is kept as a **credit-only contributor** (share 0, proof `declared`) so a later `patch announce` still says "Taught by …" but pays nobody unless the importer raises the share. `lesson.sha256` is checked against the file before anything reaches the node (`sha256 mismatch` → exit 1); a model-id mismatch with `/api/info.model` is a warning (the lesson will not fire). Nothing is announced, no ledger record is written.
- **`--contributor addr:name:share`** (repeatable, ≤ 4, Σ ≤ 1; `addr:share` without a name; share may be `25%`; share 0 = credit only) → `Contributor{role:'data_provider', proof:'declared'}` on `POST /api/patches`. `proof: 'signed'` is only produced by the browser publish flow (the node verifies the claim there); an operator-registered file cannot carry a signature the data provider did not make.
- **Node fix (outside the PR-8 file list): `PATCH /api/patches/:id` wiped `contributors` on every body that did not mention them** (the handler passed `contributors: undefined`, and `updateDraft` treats `'contributors' in patch` as "replace"). Found by `patch import`'s `PATCH {origin:'teach'}`. The handler now forwards only the keys the caller sent; `contributors: null` still clears. Covered by the new CLI import test.
- **DocsPage.** `CLI_REFERENCE.oneLiners.teach` renders as a fourth "one line" card (with a link to `/chat?teach=1`) only when the node sends it, so older nodes keep three cards; the new "Teach mode (lessons taught by visitors)" group appears in the CLI table through the existing loop. Group names stay English (the reference is rendered from the node's JSON; the surrounding page is translated).
- **Demo cluster (`scripts/cluster.mjs`).** `teach: { enabled: true, publish: 'auto', backend: TEACH_BACKEND }` is written for **node-a only** (the web-UI node; all three share one trainer container and GPU set, so one queue is enough — the lease would serialise them anyway) and only when its `config.json` is first created; existing homes (the live `~/.ainize-cluster`) are never rewritten — the script prints the `ainize config set` line instead. `TEACH_BACKEND` defaults to `'stub'` with a clearly marked `TODO(gpu)` switch to `'gradient'` once GPUs 4–6 are free of `train_rev.py`; `AINIZE_TEACH_BACKEND=gradient` overrides per run. `stubOffline` is deliberately not set (the demo checks lessons on the real model). §15 open decision 3 is thereby answered for the demo as `auto`.
- **deploy/README** documents the two host requirements the worker relies on (§8.1/§8.5): the node's Unix user in the `docker` group (`docker exec` for train / pgrep / kill, no sudo), and trainer GPUs (`teach.trainer.gpus`) disjoint from the serving GPUs with the single-slot lease semantics (`blocked: slot` when someone else's job holds them), typical durations, the `stub` / `stubOffline` fallbacks and the config block.
- **Round-trip on the dev nodes (stub backend).** node-t :3412 (stub, `stubOffline`, publish auto): key → preflight (`overlaps_listing` for the plain 픽셀플러스 phrasing after earlier e2e runs, so a run-unique phrasing was taught) → job `28cf4127…` READY in 3 s (fixture: 2,992 rows, sha `753a83e8…`) → `teach status` node / lesson-with-key / lesson-anonymous / teacher views → `save` → `lesson-pr-8-cli-187382.npz` + `recipe.json` downloaded with the token → **node-u :3413** (`init --ledger local --runtime-repo /mnt/newdata/qwen3.8 --runtime-api http://localhost:8000`) → `patch import` → DRAFT `taught-pr-8-cli-187382` (origin teach, contributor share 0, benchmark `taught/pr-8-cli-187382`, no records, ledger holds only the `node` record) → `patch apply` → `✓ applied taught-pr-8-cli-187382: 끼움: 2992행` → `ainize chat taught-pr-8-cli-187382 "픽셀플러스 종목코드 알려줘. 숫자만."` → before `058420` / after `036540` (lesson loaded in 239 ms, compare restored the rows) → `patch remove` → `뺌: 2992행`. The mechanics work end to end; the *answer* did not flip to 087600 because the stub lesson is the 픽셀플러스 fixture (rows trained on the completion phrasing `종목코드 픽셀플러스 `, which `pixelplus-087600` lists as its first benchmark sample) while the chat rendering was asked — the same fixture limitation PR-6 recorded; the template phrasing could not be measured in this session because the shared vLLM was restarting every few minutes under other jobs' load. A gradient-trained lesson is what makes the chat form fire (§8.4, still blocked on GPUs 4–6).
- **Not added:** a Playwright `cli-teach.spec.ts` — the e2e helpers point `CLI` at the main checkout and the import path is covered by `ainize-cli/test/cli.test.ts` against an in-process node plus the manual round-trip above; AZ-121 stays `automation: cli`. No gradient run (GPUs 4–6 still busy). `npm test -w packages/node` still runs `test/ain.test.ts` against the live dev chain; the PR-8 verification ran the node suites file by file without it.

### Review fixes — web (2026-08-31, after the three adversarial reviews)

Scope: `packages/web` only (plus the e2e password helper and one assertion). Findings addressed: every item of the ux-copy-i18n review that lives in the web app (header, basket placement, status pills, §8.4 durations, keep-private sheet, FAILED copy, localized times, stub honesty, key chip / stats / earnings fallback, Korean phrasings, card vs. empty state, pre-flight copy) and the spec-conformance e2e finding (`operatorToken` ignored `AINIZE_PASS`). Node-side items from the same findings (eta_s from < 3 samples, `repo_url` in the save response, `checks_failed` copy for unchecked lessons, NAME_BLOCKLIST, security findings) are **not** in this change.

Files: `components/ui/Header.tsx` · `pages/ChatPage.tsx` · `components/chat/{LessonCard,KeepPrivateSheet,LessonBasket-unchanged,MyKnowledgePanel,PreflightList,teachUtil}.tsx/ts` · `pages/TeacherPage.tsx` · `utils/useFormat.ts` (new) · `components/operator/common.tsx` (re-exports `useElapsed`) · `i18n/pages/teach.ts` · `ainize-node/e2e/helpers/ainize.ts` · `ainize-node/e2e/tests/web-teach.spec.ts` (READY assertion) · `ainize-node/e2e/results/review/review2.mjs` (verification script; screenshots `results/review2-*.png`).

- **Header at 360 px (finding 0).** `Content` is now `min-height: 81px; flex-wrap: wrap`; below `breakpoint.sm` the logo row takes the full width and the nav wraps underneath (items `8px` padding, 14 px), so `/chat`, `/signing`, `/new-patch`, `/explore`, `/teacher/:addr`, `/dashboard` (signed-in, with the operator menu) all measure `scrollWidth 360` in en and ko (`review2.json`). Desktop is unchanged (one row). Not a hamburger menu: every item stays visible and tappable.
- **Basket at the top of the picker column (finding 1).** When the node accepts lessons (`policy.enabled`) `LessonBasket` renders **above** `KnowledgePicker`; a node that does not accept lessons keeps the "does not accept" line under the picker. `?teach=1` and "Add to lesson" also scroll the basket into view when it is off-screen. Measured: basket top = 462 px at 360×780 (was 2,535 px), 327 px at 1280 (was ~1,600 px below the fold).
- **Status pill via i18n (finding 2).** `teachUtil.cardStatusKey()` maps job status → the `teach.mine.status.*` vocabulary (`EXPORTED`/`CHECKING` → new `teach.mine.status.checking` "Checking / 확인 중"; `ANNOUNCED` → "Being verified" or "On sale" by `publish_status`); the stranger's card (`teach.card.not_yours`) gets the same label. No `text-transform: uppercase` any more (the Korean pill was never uppercased; the English one no longer shouts).
- **§8.4 durations (findings 3, 4, 11).** `policyLine`: p90 < 60 s → `teach.basket.policy_open_fast` ("recent lessons finished in under a minute"); equal rounded minutes → `teach.basket.policy_open_about` ("about {p50} min"); otherwise the range. "1–1 min" cannot occur. The QUEUED card's ETA goes through `etaText()` — shown only when `policy.timing.samples ≥ 3`, "under a minute" below 90 s (the node still sends `eta_s` from fewer samples; that node-side rule is open). With `backend: 'stub'` LOADING/PREFLIGHT/first TRAINING tick say "Starting…" instead of "Warming up the model (about a minute)", and the "Times are this node's recent average" tip is hidden. `teach.pre.running` dropped the "(under 30 seconds)" promise; at pre-flight a runtime outage maps to `teach.pre.err_runtime` ("try again in a minute; your corrections are kept in this browser") via `mapTeachError(err, t, { stage: 'preflight' })` instead of "The lesson will continue automatically".
- **Keep-private sheet (findings 5, 6, 15).** `Option` grid is `20px minmax(0, 1fr)` and `Inner` has `min-width: 0`, so the hardware notice and the checkbox label are never clipped on phones; the commands live in a box (`max-height: 340px; overflow: auto`) that scrolls on its own — at 360 px the sheet's `scrollWidth` equals its width while the box scrolls to 1,160 px. The hard-coded command block (wrong `https://github.com/comcom-ai/qwen3.8` URL, no serve/hook step) is gone: after the visitor ticks "I have this hardware" the sheet fetches the node's own `RUN-LOCALLY.md` through the tokened `readme_url` and shows it verbatim ("Shown exactly as this node wrote RUN-LOCALLY.md"); "Copy commands" copies the fenced `bash` blocks. Selecting the "Run it on my own machine" radio no longer POSTs `/save` — the links are minted when the hardware box is ticked (verified: 0 POSTs after the radio, 1 after the checkbox); choosing "Download the knowledge file" still mints them (that option *is* the request for links; the "Make download links" button remains for the error path). The model name falls back to `runtime.model`, then the node's `/api/info.node.model`, instead of a constant that differed from the page's model chip.
- **FAILED card (finding 7).** `failedKey()` maps known errors to visitor sentences (`node restarted…` → `teach.card.failed_restart`, OOM → `teach.card.failed_memory`, `already_known` unchanged); any raw trainer text sits under a collapsed `<details>` "Technical details (attach when reporting)" (`data-testid="lesson-error-details"`), never in the sentence.
- **Localized times (finding 8).** `utils/useFormat.ts` hosts `useElapsed()` (moved from operator/common, which re-exports it) and a new `useDateTime()` (`toLocaleString` ko-KR / en-US). Used by `MyKnowledgePanel` ("4분 전에 만듦" — `teach.mine.created` ko is now `{ago}에 만듦`), `TeacherPage` (earnings "When" column + title) and the keep sheet's "Links expire: 2026년 9월 7일 오후 1:15".
- **Stub honesty (finding 10).** When `policy.backend === 'stub'` or `checks.note` mentions simulated/stub, the READY sentence is `teach.card.ready_sim` ("It learned it — 2 of 2 answers correct." — no "in the live model") followed by `teach.card.simulated` ("Demo node — these checks were simulated, not measured in a live model.", `data-testid="lesson-simulated"`). `web-teach.spec.ts` AZ-106 accepts both wordings and checks the note on a stub node.
- **Small copy (finding 13).** Key chip without a display name: `teach.key.chip_anon` "Teaching anonymously · 0x…" / "이름 없이 가르치는 중 · 0x…"; `teach.key.anon` is "Anonymous / 이름 없음" where a name is displayed on its own. Teacher stats tile label is "Sales / 판매" (no repeated number). Earnings table fallback maps `local-credit` → "settled as node credit" and `ain-transfer` → "settled by AIN transfer" instead of the raw scheme.
- **Korean "Suggest phrasings" (finding 12).** Templates wrap the whole question (`질문: … ? 답:`, `다음 질문에 짧게 답해줘: …?`, `다음 질문에 숫자나 이름만으로 답해줘: …?`) instead of appending a verb to a stem that may already end in a particle or a request; measured output for `픽셀플러스 (rv8cak3) 종목코드는?` is grammatical. Model-generated phrasings remain v1.1.
- **Lesson card vs. empty transcript (finding 14).** The card is the first, non-shrinking block *inside* the scrolling transcript (it used to sit outside it and squeeze the flex-1 transcript to a few pixels, so "No questions yet" peeked out under the card). The empty state now centres in the remaining space (measured: empty-state top 741 px ≥ card bottom 717 px). The card scrolls with a long transcript; it is no longer pinned — deliberate, since a tall READY card would otherwise eat the whole conversation area.
- **e2e helper (spec-conformance finding 1).** `passwordFor(node)` = `PASSWORDS[node] ?? AINIZE_PASS ?? 'e2e-pass'`, used by `operatorToken`, `loginViaUi` and `cliLogin`; `PASSWORDS` gained node-t (`http://localhost:3412` → `AINIZE_PASS_T ?? AINIZE_PASS ?? 'teach-pass'`) and NODE_A honours `AINIZE_PASS` when `AINIZE_URL` points elsewhere.
- **Verification.** `ainize-node/e2e/results/review/review2.mjs` (real stub lesson for a fresh key + mocked states) → `results/review2-{m360,d1280}-{en,ko}-*.png`, `results/review/review2.json`: no horizontal overflow on any Layout page at 360 (dashboard tables scroll inside `TableWrapper`), basket position, pill/body/simulated copy per state, `/save` request counts, keep-sheet scroll geometry, README head/sha/serve line, localized panel/teacher copy, ko phrasings. `tsc --noEmit` + `vite build` clean. e2e on node-t (`AINIZE_URL=http://localhost:3412 AINIZE_PASS=teach-pass … --project=web`): `web-teach-operator.spec.ts` 7/7, `web-teach.spec.ts` 9/9 — 15 passed + AZ-103 flaky (its first attempt ran while vLLM :8000 was unreachable, so the composer was disabled; the retry and every dependent step passed once the model server was back).

### Review fixes — core / node / CLI (2026-08-31, after the three adversarial reviews)

Files: `ainize-core/src/{catalog,config,types}.ts`, `ainize-node/src/{payouts,store,server,api,market,teach,teach-auth (new),teach-recipe,openapi,index}.ts`, `ainize-cli/src/commands/teach.ts`, tests in `ainize-core/test/core.test.ts`, `ainize-node/test/{payouts,teach}.test.ts`, `ainize-cli/test/cli.test.ts`, `deploy/README.md`. Numbers refer to the security-payout (S1–S10) and spec-conformance (C11–C20) findings plus the node-side items of the UX review.

- **S1/C18 — payouts never transfer twice.** `payouts.status` gained `paying`: every attempt first claims the row atomically (`Store.claimPayout`: `UPDATE … SET status='paying', attempts=attempts+1 WHERE status IN ('pending','failed')`, 0 rows → somebody else has it) and only then calls `wallet.transfer`. `Payouts.retry()` no longer calls `attempt()` itself — it marks the row *forced* and awaits the single serialised runner (`processPending`), so a Retry clicked while the 60-s pass is mid-transfer returns that pass's outcome instead of issuing a second AIN transfer. A row still `paying` at boot (crash between the transfer and the bookkeeping) is marked `failed` with `attempts = max` and `last_error = PAYOUT_INTERRUPTED` (`recoverInterrupted()` in `start()`): the timer never re-sends it blind; the operator confirms on chain and retries from the Payouts tab. `payoutSummary()` counts `paying` under `pending`; `GET /api/me/payouts?status=paying` is accepted. §9.3 wording "60 s timer retries" is unchanged; the `paying` state is an addition to §7.5.
- **S2 — `trust proxy` is off by default.** New `NodeConfig.server.trustProxy` (`false` | hop count | `'loopback'` | IP/CIDR list; env `AINIZE_TRUST_PROXY`; `defaultConfig` writes `server: { trustProxy: false }`). With the default, `req.ip` is the TCP peer, so `X-Forwarded-For` can no longer pick the address that per-IP quotas, bans, the policy rate limit and the operator queue's `ip` column key on. Operators behind nginx/caddy set it (deploy/README §3). Quota also no longer rests on the IP alone: **`ACTIVE_JOBS_PER_KEY = 2`** lessons in flight (QUEUED…CHECKING) per teaching key → `429 quota_key: you already have N lesson(s) in progress …`.
- **S3 — the operator cannot announce what the owner never published.** `announceJob(j, { fromPublish })`: the approve route requires `status === 'PENDING_REVIEW' && publish_status === 'pending_review'` (→ `409 job_not_ready: the owner has not published this lesson …` for a READY private draft) and, on both paths, a contributor whose `sig` verifies against the draft's `patch_sha256` / `benchmark_hash` and whose signer is the job's teaching key. `fromPublish` is passed only by `publish()` after consent + claim were recorded.
- **S4 — private drafts stay private.** (a) `GET /api/events` and `/api/patches/:id/events` redact for non-operators: teach events keep their status line but draft ids (`taught-…`) and addresses in the message are masked, `data` is reduced to `{job_id}`, and `patch` events that start with `draft ` are dropped; `/api/patches/<draft>/events` is 404 for non-operators like `/api/patches/<draft>`. The "lesson queued" line no longer contains the prompt (job name) or the key, the READY line no longer contains the draft id (both are in operator-visible `data`). (b) A DRAFT is usable in `POST /api/chat` and as `patch_ids` context in preflight / job creation **only by its owner (signed `x-ainize-auth`) or the operator** (`Market.mayUseEntry`); everyone else gets the same 404 / `invalid: unknown knowledge` as for a non-existent id. **Web follow-up needed:** `SIGNED_ENDPOINTS` in `ainize-web/src/api/api.ts` must include `'chat'` (the "Try it now" live test of a private draft now needs the signature; published lessons are unaffected). (c) `GET /api/teacher/:address` lists PENDING_REVIEW lessons by **job id**, not draft id, and never lists DRAFT catalog entries.
- **S5 — lineage over-payment is impossible.** `royaltySplit` clamps every share to [0, 1] and caps the carve of one anchor at the slice it is carved from (Σ payouts ≤ amount whatever a peer anchor carries); `Market.catalogAll()` runs `sanitizeContributors()` (non-throwing `validateContributors`) on every ledger anchor and drops a malformed list; `settlePayment` additionally refuses to distribute more than the price (falls back to seller-only with an `error` event). Core test: a peer anchor with `share: 5` yields `{evil: 3, node: 7}` for a 10 AIN sale.
- **S6 — a lesson can no longer stay applied.** New `teach_jobs.lesson_applied` column (additive `ALTER TABLE`), set *before* `applyRaw(lesson)` in CHECKING and cleared after a successful `removeRaw`. `recoverAfterRestart()` no longer flips CHECKING → EXPORTED blindly: the job (and any row still flagged) goes to `pendingRestore`; `tick()` runs `restoreTable()` first — remove the lesson if it is still on the table, remove context blobs that are not operator-pinned, `reassertPinned()`, clear the flag, then EXPORTED — and starts no other step while the table may be dirty (retried until the model server answers). `cleanupFiles()` refuses to delete the npz of a flagged job. `TeachWorker.stop()` waits up to 90 s (was 10 s) and a running check aborts at its next model call (`askChat`/`askRaw` throw `node stopping`), so the `finally` restores the table before the process exits.
- **S7 — attribution follows the signer.** `creditedAddress(c)` = `signer` for `proof: 'declared'`, else `address`; used by `/api/teacher/:address` (lessons) and `GET /api/catalog?contributor=`. A declared payout wallet only receives money (its earnings still appear on its page) — it is never shown as the teacher. `publish-challenge` refuses `payout_address` = the node's own address; `createDraft`/`updateDraft` refuse a contributor equal to the node.
- **S8 — request-bound, single-use visitor auth** (`ainize-node/src/teach-auth.ts`). New header form `<address>:<ts>:<sig>:v2` with `sig = signMessage("teach:<nodeAddress>:<METHOD>:<path+query>:<ts>[:<sha256(raw body)>]")` (`express.json({ verify })` keeps the raw bytes); every accepted header is remembered for the 5-min window, so a replay is refused. The legacy `teach:<ts>` form is still accepted (the web app sends it) but an exact replay (same header, same method + path) is refused; it remains replayable across routes until clients move to v2 — `teachAuthHeaderFor()` is exported from `@ainize/node`, the CLI (`signedTeachHeader`, `nodeAddressOf`) signs v2 everywhere. §6.1 wording ("`sig = signMessage("teach:<ts>")`") is thereby superseded for new clients.
- **S9 — 4xx, not 500.** Core `ValidationError` (thrown by `validateContributors`, new `validatePrice` with `PRICE_RE = /^\d+(\.\d+)?$/`), node `ConflictError` (duplicate id, editing a non-draft) and `NotFoundError` are mapped to 400 / 409 / 404 in the API error handler; `POST`/`PATCH /api/patches` validate `price` with zod as well (negative or non-numeric prices are refused everywhere, including the CLI import path).
- **S10 — preflight cost.** `TeachWorker.preflightUnits()` = ⌈(facts + distinct context ids) / 3⌉ live-test units (≥ 1), charged to the IP **and** the key bucket (`Market.chatQuota(…, units)`); `createJob` re-checks queue_max and the per-key cap synchronously right before the insert (the earlier check sits before several awaits).
- **C11 — graceful stop during TRAINING requeues.** `stop()` sets `stopped` *before* SIGTERM-ing the docker client and `killStray()`s the in-container process; `runProcess`/`runStub` return `node stopping`, `runJob` maps it to QUEUED (or EXPORTED from CHECKING) without touching the once-only restart marker. Test: stop during the `hang` scenario on a second in-process node → row QUEUED, `docker exec flashtrain kill -TERM 4242` observed, marker null; a fabricated crash mid-CHECKING is restored at the next start.
- **C12 — e2e helper**: fixed in the working tree by the web review job (`passwordFor`, node-t entry); not part of this commit.
- **C13 — seller skip is case-insensitive** in both royaltySplit passes (test with a mixed-case seller address).
- **C14 — `publish_status: 'listed'` is real.** `reconcilePublished()` (every 30 s from `tick()`) moves `announced` → `listed` once the catalog shows the anchor LISTED / SUPERSEDED / CHALLENGED (verifier REJECTED is left as `announced`; the catalog status is the truth for that). OpenAPI enum documented.
- **C15 — expiry covers declined and failed lessons.** `reject()` sets `expires_at = now + draftTtlDays`; `sweepExpired()` expires READY / NEEDS_MORE / REJECTED past their expiry (draft, files, tokens removed; `reject_reason` kept) and removes the job dir of FAILED / CANCELLED rows older than the TTL (status unchanged). Flagged (`lesson_applied`) rows are skipped.
- **C16 — honest error for unchecked lessons.** `publish-challenge` / `publish` on a READY lesson with `checks.executed === false` now answer `409 job_not_ready: this lesson has not been measured in the live model yet — run a re-check first` (was `checks_failed …`).
- **C17 — chunked stdout covered.** Fake spawn scenario `chunked` (JSON `done` in three chunks, a plain-text progress line in between, no trailing newline) — which exposed that `runProcess` judged the exit before readline had flushed the last line; it now waits for the reader's `close` (≤ 2 s) after the child exits.
- **C19 — hidden names** are redacted on `GET /api/chat/patches.items` too.
- **C20 — node-t pidfile** rewritten from the listener on restart (see below).
- **UX (node side).** §8.4 minimum-sample rule: `eta_s` (job view) and `queue.position_eta_s` (policy) are `null` until ≥ `ETA_MIN_SAMPLES = 3` measured lessons. `NAME_BLOCKLIST` rejects Unicode bidi / zero-width / format controls (U+00AD, U+200B–200F, U+2028–202E, U+2060–206F, U+FEFF, …) and names are NFKC-normalised with whitespace collapsed before storing (`normalizeDisplayName`). Simulated checks are flagged machine-readably: `checks.simulated: true` and `policy.simulated_checks: true` on stub-offline nodes (the free-text `checks.note` stays). `POST …/save` returns `repo_url` (`LOCAL_RUN_REPO_URL`, the same repo RUN-LOCALLY.md clones) and `model_id` so the keep-private sheet can build its command block from the node's values.
- **Tests / verification.** core 16, node payouts 10 (race, interrupted row), teach 22 (v2/legacy auth matrix, consent gate, event redaction, draft gate, attribution, per-key cap, eta rule, decline expiry, listed transition, chunked stdout, second-node stop/restore), chat + cluster 12, CLI 15 — all green; `ain.test.ts` not run (shared chain).

### Verification pass (2026-08-31, after the two review-fix commits)

Files: `ainize-web/src/lib/teacherKey.ts`, `ainize-web/src/api/api.ts`, `ainize-web/test/teacherKey.test.ts`, `ainize-node/src/market.ts`.

- **Web sends the request-bound v2 header (closes the S8 transition gap and the S4 web follow-up).** `teacherKey.ts` gained `teachAuthMessage` / `authHeaderV2` / `teachAuthHeaderFor` — byte-identical to the node's `teach-auth.ts` builder (`teach:<node>:<METHOD>:<path+query>:<ts>[:<sha256 body>]`, suffix `:v2`; unit test compares the two builders and drives `TeachAuth.verify` through single-use / route / body / node binding). `api.ts` no longer signs in `prepareHeaders` (which cannot see the final URL or body): signed endpoints carry an internal marker and a custom `fetchFn` (`signedFetch`) signs the actual `Request` — method, path + query, raw body — with the node address read once from `/api/info`; the legacy `teach:<ts>` header is only the fallback when that address cannot be read. `SIGNED_ENDPOINTS` now includes **`chat`**, so "Try it now" on a private draft (`POST /api/chat` with a `taught-…` id) authenticates as the owner; published lessons are unaffected. Legacy acceptance on the node is unchanged (CLI and web both send v2 now; the legacy form stays for scripts until the next release).
- **`Market.chat` checks visibility before the runtime.** The owner / operator gate (`mayUseEntry`) for every requested id now runs *before* `runtime.status()`, so a non-owner probing a private draft gets the same `404 patch not found` whether or not the model server is up (before: `500 serving API unreachable` while vLLM was down — no leak, but a different error shape than `GET /api/patches/:id`).
