...
Files: `packages/cli/src/commands/teach.ts` (new: `ainize teach status`), `packages/cli/src/commands/patch.ts` (`parseContributors`, `draftFromRecipe`, `patchImport`, `PublishArgs.contributor`), `packages/cli/src/bin.ts` (`teach status [target] [--key --key-file]`, `patch import <file> --recipe`, `--contributor` on `publish` and `patch publish`), `packages/cli/src/index.ts`, `packages/cli/test/cli.test.ts` (+3 tests, node started with `teach.enabled` + stub + `stubOffline`), `packages/node/src/openapi.ts` (`CLI_REFERENCE`: teach one-liner, "Teach mode" group, `--contributor` line), `packages/node/src/api.ts` (`PATCH /api/patches/:id` fix, below), `packages/web/src/pages/DocsPage.tsx` + `i18n/pages/docs.ts` (fourth one-liner card "Teaching the model", en+ko), `deploy/README.md` (docker group + GPU allocation), `scripts/cluster.mjs` (teach demo config + `TEACH_BACKEND` switch), `README.md` (terminal lines), `docs/ux-test-scenarios.json` (AZ-121 rewritten with the real commands; md/html re-rendered).
...
- **Not added:** a Playwright `cli-teach.spec.ts` — the e2e helpers point `CLI` at the main checkout and the import path is covered by `packages/cli/test/cli.test.ts` against an in-process node plus the manual round-trip above; AZ-121 stays `automation: cli`. No gradient run (GPUs 4–6 still busy). `npm test -w packages/node` still runs `test/ain.test.ts` against the live dev chain; the PR-8 verification ran the node suites file by file without it.
...
Files: `components/ui/Header.tsx` · `pages/ChatPage.tsx` · `components/chat/{LessonCard,KeepPrivateSheet,LessonBasket-unchanged,MyKnowledgePanel,PreflightList,teachUtil}.tsx/ts` · `pages/TeacherPage.tsx` · `utils/useFormat.ts` (new) · `components/operator/common.tsx` (re-exports `useElapsed`) · `i18n/pages/teach.ts` · `packages/e2e/helpers/ainize.ts` · `packages/e2e/tests/web-teach.spec.ts` (READY assertion) · `packages/e2e/results/review/review2.mjs` (verification script; screenshots `results/review2-*.png`).
...
Files: `packages/web/src/lib/teacherKey.ts`, `packages/web/src/api/api.ts`, `packages/web/test/teacherKey.test.ts`, `packages/node/src/market.ts`.
