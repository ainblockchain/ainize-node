...
- **Teaching key in the CLI.** `--key-file <ainize-teaching-key-….json>` (the browser's backup file, same shape as `teacherKeyBackup`), `--key <64-hex>` or `NGRAM_TEACH_KEY` (hex or a path). The CLI signs `x-ngram-auth` exactly like the web app (`<address>:<ts>:<sig over "teach:<ts>">`, core `signMessage`) — with it the lesson view is the owner's full body (facts, before/after, checks, draft/published id) and the node view lists "your lessons on this node"; without it the node returns status only (§6.1) and the CLI says so. The CLI never creates a key: teaching stays a browser flow (`/chat?teach=1`), as in §4.
...
- **Demo cluster (`scripts/cluster.mjs`).** `teach: { enabled: true, publish: 'auto', backend: TEACH_BACKEND }` is written for **node-a only** (the web-UI node; all three share one trainer container and GPU set, so one queue is enough — the lease would serialise them anyway) and only when its `config.json` is first created; existing homes (the live `~/.ngram-cluster`) are never rewritten — the script prints the `ainize config set` line instead. `TEACH_BACKEND` defaults to `'stub'` with a clearly marked `TODO(gpu)` switch to `'gradient'` once GPUs 4–6 are free of `train_rev.py`; `NGRAM_TEACH_BACKEND=gradient` overrides per run. `stubOffline` is deliberately not set (the demo checks lessons on the real model). §15 open decision 3 is thereby answered for the demo as `auto`.
