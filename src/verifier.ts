  ...
  async verifyOne(anchor: PatchAnchor, opts: { recheck?: boolean; quiet?: boolean } = {}): Promise<Attestation> {
    ...
    if (mine && mine.created_at >= challengedAt) {
      ...
      if (!canUpgrade && !opts.recheck) {
        throw new ConflictError(`this node already attested ${anchor.id} (${mine.passed ? 'PASS' : 'FAIL'}, ${mine.verified_on}, ${new Date(mine.created_at).toISOString()}). To measure it again and put the result on the record WITHOUT taking it off sale, re-run it as a recheck (ainize patch verify ${anchor.id} --recheck). A recheck that fails withdraws this node's earlier PASS; one that passes is a visible confirmation. A challenge (ainize patch challenge ${anchor.id} --reason …) stops the seller's sales and asks every verifier to re-run it${mine.verified_on === 'hash-only' ? '; this node will also re-verify on its own once it has a model server that can run the benchmark (it has none that matches now)' : ''}.`);
      }
    }
