...
def collect(report_path):
    ...
    for sid in sorted(rows):
        n = int(sid.split('-')[1])
        # teach mode owns AZ-101…AZ-122 (teach v1), AZ-123…AZ-222 (the dataset door, reported in
        # docs/ux-test-results-dataset.md) and the TM-* pair; every other AZ id is a numbered scenario of the main
        # catalogue — AZ-223/224/225/226 (the four chat scenarios, renumbered out of the dataset block) included.
        (teach if sid.startswith('TM-') or 101 <= n <= 222 else hundred)[sid] = rows[sid]
