...
#   scripts/cluster-restart.sh --fresh  # wipe ~/.ainize-cluster first
#   scripts/cluster-restart.sh --stop   # stop only
#   AINIZE_CLUSTER_HOME=/tmp/c AINIZE_PORT_BASE=3502 scripts/cluster-restart.sh   # restart a private cluster only
...
# THIS cluster home (AINIZE_HOME=<home>/node-*, AINIZE_CLUSTER_HOME=<home>, or the default home when unset) are signalled.
set -uo pipefail
ROOT="$(cd "$(dirname "$0")/.." && pwd)"
DEFAULT_HOME="$HOME/.ainize-cluster"
HOME_DIR="${AINIZE_CLUSTER_HOME:-$DEFAULT_HOME}"
PORT_BASE="${AINIZE_PORT_BASE:-3402}"
...
owns_home() {   # $1 = pid — true when the process belongs to the cluster rooted at HOME_DIR
  ...
  local h; h=$(printf '%s\n' "$env" | sed -n 's/^AINIZE_HOME=//p' | head -1)
  if [[ -n "$h" ]]; then [[ "$h" == "$HOME_DIR"/* ]]; return; fi
  local c; c=$(printf '%s\n' "$env" | sed -n 's/^AINIZE_CLUSTER_HOME=//p' | head -1)
